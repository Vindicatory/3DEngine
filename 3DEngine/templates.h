...
// TODO Move colors
inline SDL_Color COLOR_GREEN {0, 255, 0, 255};
inline SDL_Color COLOR_RED{255, 0, 0, 255};
inline SDL_Color COLOR_BLUE{0, 0, 255, 255};
...
concept Numeric = Comparable<T> &&
   requires (T a, T b) {
   a++; a--; a + b; a - b; a * b; a / b; a *= b; a /= b; a = 1;
};
...
struct engPoint3D {
   number x, y, z;
   number w = 1; // Need a 4th term to perform sensible matrix vector multiplication
...
{
   plane_n.Normalize();
   float plane_d = -1 * plane_n.DotProduct(plane_p);
